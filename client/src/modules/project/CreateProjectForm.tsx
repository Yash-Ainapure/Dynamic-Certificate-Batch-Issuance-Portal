  ...
  return (
    <form className="grid gap-6 md:grid-cols-2" onSubmit={onSubmit}>
      <div className="space-y-3">
        <input data-tour="project-name" className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2 w-full rounded" placeholder="Project Name" value={name} onChange={(e) => setName(e.target.value)} required />
        ...
        <Button data-tour="project-submit" type="submit" disabled={!template || !picked || submitting}>{submitting ? 'Creating…' : 'Create Project'}</Button>
